...
- Current: paused/final-for-now playable baseline with a fallback crash-report diagnostic follow-up available from GitHub Actions run `26675725620`. The app has the `Streets of Rage` display name, app icon, virtual joystick, pillar-safe CFG button, right-side Start/Back buttons, persistent custom layout settings, and visible crash reports.
