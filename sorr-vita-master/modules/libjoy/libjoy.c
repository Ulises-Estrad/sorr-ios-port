...
static int portable_joy_num_logged = 0;
static int portable_joy_invalid_log_count = 0;
...
static void portable_joy_log_invalid( const char * func, int joy, int a, int b )
{
    if ( portable_input_diag() && portable_joy_invalid_log_count < 120 )
    {
        fprintf( stderr, "[PORTABLE:INPUT] %s invalid joy=%d selected=%d max=%d a=%d b=%d\n", func, joy, _selected_joystick, _max_joys, a, b );
        portable_joy_invalid_log_count++;
    }
}
#endif
...
{
#ifdef PORTABLE_RUNTIME_DIAG
    if ( portable_input_diag() && !portable_joy_num_logged )
    {
        fprintf( stderr, "[PORTABLE:INPUT] JOY_NUMBER max=%d selected=%d\n", _max_joys, _selected_joystick );
        portable_joy_num_logged = 1;
    }
#endif
    return _max_joys ;
}
...
{
#ifdef PORTABLE_RUNTIME_DIAG
    if ( portable_input_diag() )
    {
        fprintf( stderr, "[PORTABLE:INPUT] JOY_SELECT request=%d max=%d\n", joy, _max_joys );
    }
#endif
...
#ifdef PORTABLE_RUNTIME_DIAG
    portable_joy_log_invalid( "JOY_BUTTONS", _selected_joystick, -1, -1 );
#endif
    return 0 ;
}
...
#ifdef PORTABLE_RUNTIME_DIAG
    portable_joy_log_invalid( "JOY_AXES", _selected_joystick, -1, -1 );
#endif
    return 0 ;
}
...
#ifdef PORTABLE_RUNTIME_DIAG
    portable_joy_log_invalid( "JOY_GETBUTTON", _selected_joystick, button, -1 );
#endif
    return 0 ;
}
...
#ifdef PORTABLE_RUNTIME_DIAG
    portable_joy_log_invalid( "JOY_GETAXIS", _selected_joystick, axis, -1 );
#endif
    return 0 ;
}
...
void  __bgdexport( libjoy, module_initialize )()
{
    ...
    if ( portable_input_diag() )
    {
        fprintf( stderr, "[PORTABLE:INPUT] SDL_NumJoysticks=%d\n", _max_joys );
    }
#endif
    ...
    for ( i = 0; i < _max_joys; i++ )
    {
        ...
        if ( portable_input_diag() )
        {
            fprintf( stderr, "[PORTABLE:INPUT] joystick index=%d name=%s opened=%d axes=%d buttons=%d hats=%d\n",
                     i,
                     SDL_JoystickNameForIndex( i ) ? SDL_JoystickNameForIndex( i ) : "(null)",
                     _joysticks[i] ? 1 : 0,
                     _joysticks[i] ? SDL_JoystickNumAxes( _joysticks[i] ) : 0,
                     _joysticks[i] ? SDL_JoystickNumButtons( _joysticks[i] ) : 0,
                     _joysticks[i] ? SDL_JoystickNumHats( _joysticks[i] ) : 0 );
        }
#endif
    }
