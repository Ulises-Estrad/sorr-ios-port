...
static int gr_object_x64_store_handle( OBJECT * object )
{
    int i;

    if ( !object ) return 0;

    for ( i = 1; i < GR_OBJECT_X64_HANDLE_TABLE_SIZE; i++ )
    {
        if ( !gr_object_x64_handles[i] )
        {
            gr_object_x64_handles[i] = object;
#ifdef PORTABLE_RUNTIME_DIAG
            PORTABLE_DIAG_LOG( "RENDER", "x64 object handle store handle=%d object=%p z=%d", i, ( void * )object, object->z );
#endif
            return i;
        }
    }
...
#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "RENDER", "x64 object handle table full object=%p z=%d", ( void * )object, object->z );
#endif
    return 0;
}

static OBJECT * gr_object_x64_get_handle( int id, const char * op )
{
    OBJECT * object = NULL;

    if ( id > 0 && id < GR_OBJECT_X64_HANDLE_TABLE_SIZE )
        object = gr_object_x64_handles[id];

#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "RENDER", "x64 object handle get op=%s handle=%d object=%p", op ? op : "(null)", id, ( void * )object );
#endif

    return object;
}

static OBJECT * gr_object_x64_release_handle( int id, const char * op )
{
    OBJECT * object = gr_object_x64_get_handle( id, op );

    if ( object && id > 0 && id < GR_OBJECT_X64_HANDLE_TABLE_SIZE )
        gr_object_x64_handles[id] = NULL;

#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "RENDER", "x64 object handle release op=%s handle=%d object=%p", op ? op : "(null)", id, ( void * )object );
#endif

    return object;
}
#else
#define gr_object_x64_store_handle(object) (( int )( object ))
#define gr_object_x64_get_handle(id,op) (( OBJECT * )( id ))
#define gr_object_x64_release_handle(id,op) (( OBJECT * )( id ))
#endif
...
{
    CONTAINER * ctr ;
    OBJECT * object = gr_object_x64_release_handle( id, "destroy" ) ;
