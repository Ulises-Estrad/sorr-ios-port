...
void gr_unlock_screen()
{
    ...
    SDL_RenderPresent(renderer);
    {
        ...
        static int timing_last_tick = 0;
        static int timing_last_present = 0;
        int now = SDL_GetTicks();
        present_count++;
        if ( present_count <= 20 || ( present_count % 1000 ) == 0 )
            PORTABLE_DIAG_LOG( "VIDEO", "frame present count=%d", present_count );
        if ( PORTABLE_DIAG_ENV_ON( "SORR_PORTABLE_DIAG_TIMING" ) )
        {
            if ( !timing_last_tick ) timing_last_tick = now;
            if ( now - timing_last_tick >= 1000 )
            {
                int elapsed = now - timing_last_tick;
                int presents = present_count - timing_last_present;
                float present_fps = elapsed > 0 ? ( ( float )presents * 1000.0f ) / ( float )elapsed : 0.0f;
                PORTABLE_DIAG_LOG( "TIMING", "present_summary elapsed_ms=%d presents=%d present_fps=%.2f waitvsync=%d", elapsed, presents, present_fps, waitvsync );
                timing_last_tick = now;
                timing_last_present = present_count;
            }
        }
    }
}
