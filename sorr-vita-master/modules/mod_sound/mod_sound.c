...
static Mix_Chunk * modsound_x64_chunk_ptr( int handle, const char * op )
{
    Mix_Chunk * chunk = NULL;

    if ( handle > 0 && handle < MODSOUND_X64_HANDLE_TABLE_SIZE )
...
static Mix_Chunk * modsound_x64_chunk_release( int handle, const char * op )
{
...
static int modsound_x64_music_handle( Mix_Music * music, const char * op )
{
    int i;
    ...
    for ( i = 1; i < MODSOUND_X64_HANDLE_TABLE_SIZE; i++ )
    {
        if ( modsound_x64_music[i] == music ) return i;
    }

    for ( i = 1; i < MODSOUND_X64_HANDLE_TABLE_SIZE; i++ )
    {
        if ( !modsound_x64_music[i] )
        {
            modsound_x64_music[i] = music;
#ifdef PORTABLE_RUNTIME_DIAG
            PORTABLE_DIAG_LOG( "AUDIO", "x64 music handle store op=%s handle=%d music=%p", op ? op : "(null)", i, ( void * )music );
#endif
            return i;
        }
    }
...
#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "AUDIO", "x64 music handle table full op=%s music=%p", op ? op : "(null)", ( void * )music );
#endif
    return 0;
}

static Mix_Music * modsound_x64_music_ptr( int handle, const char * op )
{
    Mix_Music * music = NULL;

    if ( handle > 0 && handle < MODSOUND_X64_HANDLE_TABLE_SIZE )
        music = modsound_x64_music[handle];

#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "AUDIO", "x64 music handle get op=%s handle=%d music=%p", op ? op : "(null)", handle, ( void * )music );
#endif
    return music;
}

static Mix_Music * modsound_x64_music_release( int handle, const char * op )
{
    Mix_Music * music = modsound_x64_music_ptr( handle, op );

    if ( music && handle > 0 && handle < MODSOUND_X64_HANDLE_TABLE_SIZE )
        modsound_x64_music[handle] = NULL;

#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "AUDIO", "x64 music handle release op=%s handle=%d music=%p", op ? op : "(null)", handle, ( void * )music );
#endif
    return music;
}
#else
...
static int sound_init()
{
    ...
    if ( PORTABLE_DIAG_ENV_ON( "SORR_PORTABLE_AUDIO_STUB" ) )
    {
        PORTABLE_DIAG_LOG( "AUDIO", "sound_init stubbed by SORR_PORTABLE_AUDIO_STUB" );
        audio_initialized = 1;
        return 0;
    }
    ...
    if ( !audio_initialized )
    {
        ...
        PORTABLE_DIAG_LOG( "AUDIO", "Mix_OpenAudio begin rate=%d format=%u channels=%d buffers=%d", audio_rate, audio_format, audio_channels, audio_buffers );
        if ( Mix_OpenAudio( audio_rate, audio_format, audio_channels, audio_buffers ) >= 0 )
        {
            ...
            audio_initialized = 1;
            PORTABLE_DIAG_LOG( "AUDIO", "Mix_OpenAudio ok rate=%d format=%u channels=%d mix_channels=%d", audio_rate, audio_format, audio_channels, audio_mix_channels );
            return 0;
        }
    }

    PORTABLE_DIAG_LOG( "AUDIO", "Mix_OpenAudio failed error=%s", SDL_GetError() );
...
static int load_song( const char * filename )
{
    ...
    return modsound_x64_music_handle( music, "LOAD_MUSIC" );
}
...
//    SDL_Log("Called play_song();");
    ...
    {
        Mix_Music * music = modsound_x64_music_ptr( id, "PLAY_MUSIC" );
        if ( !music ) return -1;
...
static int fade_music_in( int id, int loops, int ms )
{
    if ( audio_initialized && id )
    {
        Mix_Music * music = modsound_x64_music_ptr( id, "FADE_MUSIC_IN" );
        if ( !music ) return -1;
...
static int unload_song( int id )
{
    if ( audio_initialized && id )
    {
        ...
        Mix_Music * music = modsound_x64_music_release( id, "UNLOAD_MUSIC" );
...
static int  set_wav_volume( int sample, int volume )
{
    ...
    if ( sample )
    {
        Mix_Chunk * chunk = modsound_x64_chunk_ptr( sample, "SET_WAV_VOLUME" );
        if ( !chunk ) return -1;
...
void  __bgdexport( mod_sound, module_initialize )()
{
    ...
    if ( PORTABLE_DIAG_ENV_ON( "SORR_PORTABLE_AUDIO_STUB" ) )
    {
        PORTABLE_DIAG_LOG( "AUDIO", "module_initialize audio subsystem skipped by stub env" );
        return;
    }
    PORTABLE_DIAG_LOG( "AUDIO", "SDL audio subsystem init begin was_init=%u", SDL_WasInit( SDL_INIT_AUDIO ) );
    if ( !SDL_WasInit( SDL_INIT_AUDIO ) ) SDL_InitSubSystem( SDL_INIT_AUDIO );
    PORTABLE_DIAG_LOG( "AUDIO", "SDL audio subsystem init end was_init=%u error=%s", SDL_WasInit( SDL_INIT_AUDIO ), SDL_GetError() );
#endif
}
