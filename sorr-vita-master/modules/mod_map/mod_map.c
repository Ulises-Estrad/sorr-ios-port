...
static int modmap_x64_palette_handle( PALETTE * pal, const char * op )
{
    int i;
    ...
    for ( i = 1; i < MODMAP_X64_PALETTE_TABLE_SIZE; i++ )
    {
        ...
        {
#ifdef PORTABLE_RUNTIME_DIAG
            PORTABLE_DIAG_LOG( "PALETTE", "x64 palette handle reuse op=%s handle=%d pal=%p", op ? op : "(null)", i, ( void * )pal );
#endif
            return i;
        }
    }
    ...
    for ( i = 1; i < MODMAP_X64_PALETTE_TABLE_SIZE; i++ )
    {
        if ( !modmap_x64_palettes[i] )
        {
            modmap_x64_palettes[i] = pal;
#ifdef PORTABLE_RUNTIME_DIAG
            PORTABLE_DIAG_LOG( "PALETTE", "x64 palette handle store op=%s handle=%d pal=%p", op ? op : "(null)", i, ( void * )pal );
#endif
            return i;
        }
    }
...
#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "PALETTE", "x64 palette handle table full op=%s pal=%p", op ? op : "(null)", ( void * )pal );
#endif
    return 0;
}

static PALETTE * modmap_x64_palette_ptr( int handle, const char * op )
{
    PALETTE * pal = NULL;

    if ( handle > 0 && handle < MODMAP_X64_PALETTE_TABLE_SIZE )
        pal = modmap_x64_palettes[handle];

#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "PALETTE", "x64 palette handle get op=%s handle=%d pal=%p", op ? op : "(null)", handle, ( void * )pal );
#endif

    return pal;
}

PALETTE * modmap_x64_palette_from_handle( int handle, const char * op )
{
    return modmap_x64_palette_ptr( handle, op );
}

static PALETTE * modmap_x64_palette_release( int handle, const char * op )
{
    PALETTE * pal = modmap_x64_palette_ptr( handle, op );

    if ( pal && handle > 0 && handle < MODMAP_X64_PALETTE_TABLE_SIZE )
        modmap_x64_palettes[handle] = NULL;

#ifdef PORTABLE_RUNTIME_DIAG
    PORTABLE_DIAG_LOG( "PALETTE", "x64 palette handle release op=%s handle=%d pal=%p", op ? op : "(null)", handle, ( void * )pal );
#endif
...
#else
#define modmap_x64_pointer_param(cell) (( void * )( *( cell ) ))
#endif
...
{
    GRAPH * bmp ;
    int * out_x = ( int * )modmap_x64_pointer_param( &params[3] );
    int * out_y = ( int * )modmap_x64_pointer_param( &params[4] );
    ...
    *out_x = bmp->cpoints[params[2]].x ;
    *out_y = bmp->cpoints[params[2]].y ;
    return 1 ;
}
...
{
    PALETTE * pal = modmap_x64_palette_ptr( params[1], "SAVE_PAL" );
    int r;
    if ( params[1] && !pal ) return 0;
    r = gr_save_pal( string_get( params[0] ), pal );
    string_discard( params[0] );
    return r;
}
...
{
    return modmap_x64_palette_handle( pal_new(( PALETTE * )NULL ), "PAL_NEW" ) ;
}
...
{
    PALETTE * base = modmap_x64_palette_ptr( params[0], "PAL_CLONE" );
    if ( params[0] && !base ) return 0;
    return modmap_x64_palette_handle( pal_new( base ), "PAL_CLONE" ) ;
}
...
{
    PALETTE * pal = modmap_x64_palette_release( params[0], "PAL_UNLOAD" );
    if ( params[0] && !pal ) return 0;
    pal_destroy( pal ) ;
    return 1;
}
...
{
    PALETTE * pal = modmap_x64_palette_ptr( params[0], "PAL_REFRESH" );
    if ( params[0] && !pal ) return 0;
    pal_refresh( pal );
    return 1;
}
...
{
    PALETTE * pal = modmap_x64_palette_ptr( params[2], "PAL_MAP_ASSIGN" );
    if ( params[2] && !pal ) return 0;
    return pal_map_assign( params[0], params[1], pal );
}
...
static int modmap_pal_map_getid( INSTANCE * my, int * params )
{
    ...
    return modmap_x64_palette_handle( bmp->format->palette, "PAL_GETID" ) ;
}
...
{
    PALETTE * pal = modmap_x64_palette_ptr( params[0], "SET_SYSTEM_PAL" );
    if ( params[0] && !pal ) return 0;
    if ( pal_set(( PALETTE * )NULL, 0, 256, ( uint8_t * )pal->rgb ) )
    {
...
{
    uint8_t * rawpal = ( uint8_t * )modmap_x64_pointer_param( &params[0] );
    if ( !rawpal ) return 0;
    if ( pal_set(( PALETTE * )NULL, 0, 256, rawpal ) )
    {
        pal_refresh( sys_pixel_format->palette );
        return 1;
    }
...
{
    PALETTE * pal = modmap_x64_palette_ptr( params[0], "PAL_SET" );
    int ret;
    if ( params[0] && !pal ) return 0;
    uint8_t * rawpal = ( uint8_t * )modmap_x64_pointer_param( &params[3] );
    if ( !rawpal ) return 0;
    ret = pal_set( pal, params[1], params[2], rawpal ) ;
...
{
    PALETTE * pal = modmap_x64_palette_ptr( params[0], "PAL_GET" );
    if ( params[0] && !pal ) return 0;
    uint8_t * rawpal = ( uint8_t * )modmap_x64_pointer_param( &params[3] );
    if ( !rawpal ) return 0;
    return ( pal_get( pal, params[1], params[2], rawpal ) ) ;
}
