...
int gr_set_mode( int width, int height, int depth )
{
    ...
    PORTABLE_DIAG_LOG( "VIDEO", "gr_set_mode begin requested=%dx%dx%d window=%p renderer=%p", width, height, depth, window, renderer );
    ...
    if(!window && !renderer) {
        ...
        PORTABLE_DIAG_LOG( "VIDEO", "SDL_CreateWindow begin title=%s size=%dx%d flags=%d", apptitle ? apptitle : "(null)", surface_width, surface_height, sdl_flags );
        window = SDL_CreateWindow(apptitle,
                                  SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                  surface_width, surface_height, sdl_flags);
        PORTABLE_DIAG_LOG( "VIDEO", "SDL_CreateWindow end window=%p error=%s", window, SDL_GetError() );
        if(window) {
            sdl_flags = 0;
            if (waitvsync) {
                sdl_flags = SDL_RENDERER_PRESENTVSYNC;
            }
            PORTABLE_DIAG_LOG( "VIDEO", "SDL_CreateRenderer begin flags=%d", sdl_flags );
            renderer = SDL_CreateRenderer(window, -1, sdl_flags);
            PORTABLE_DIAG_LOG( "VIDEO", "SDL_CreateRenderer end renderer=%p error=%s", renderer, SDL_GetError() );
    ...
    PORTABLE_DIAG_LOG( "VIDEO", "initial SDL_RenderPresent done" );
    ...
    texture = SDL_CreateTexture(renderer,
                                   ...
                                   width, height);
    PORTABLE_DIAG_LOG( "VIDEO", "SDL_CreateTexture end texture=%p error=%s", texture, SDL_GetError() );
...
//    gr_rects_clear();
    ...
    PORTABLE_DIAG_LOG( "VIDEO", "gr_set_mode end screen=%p scr=%dx%d depth=%d", screen, scr_width, scr_height, sys_pixel_format ? sys_pixel_format->depth : -1 );
    return 0;
}
...
void __bgdexport( libvideo, module_initialize )()
{
    ...
    PORTABLE_DIAG_LOG( "VIDEO", "SDL video subsystem init begin was_init=%u", SDL_WasInit( SDL_INIT_VIDEO ) );
    if ( !SDL_WasInit( SDL_INIT_VIDEO ) ) SDL_InitSubSystem( SDL_INIT_VIDEO );
    PORTABLE_DIAG_LOG( "VIDEO", "SDL video subsystem init end was_init=%u error=%s", SDL_WasInit( SDL_INIT_VIDEO ), SDL_GetError() );
