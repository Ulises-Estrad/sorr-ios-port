...
#define SORR_IOS_BUILD_LABEL "ios-touch-dualsense-bundled"
#endif

#ifndef SORR_IOS_ARTIFACT_LABEL
#define SORR_IOS_ARTIFACT_LABEL "ios-touch-dualsense-bundled-device-arm64"
#endif
...
#include "sorr_ios_bundled_data.h"
...
static int sorr_ios_prepare_data_layout(sorr_ios_data_layout *layout)
{
    ...
    SDL_free(base_path);
    if (!sorr_ios_join_path(layout->game_bundle_root, sizeof(layout->game_bundle_root), layout->bundle_root, "GameData")) return 0;
    layout->bundled_data = sorr_ios_dir_contains_sorr_dat(layout->game_bundle_root);
    ...
    if (!sorr_ios_join_path(documents_dir, sizeof(documents_dir), home, "Documents") ||
        ...
        !sorr_ios_join_path(layout->support_root, sizeof(layout->support_root), app_support_dir, layout->bundled_data ? "SORR-Bundled" : "SORR") ||
    ...
    if (!sorr_ios_mkdir_if_needed(documents_dir) ||
        ...
        (!layout->bundled_data && !sorr_ios_mkdir_if_needed(layout->documents_import_dir)) ||
                           ...
                           "README_DIAGNOSTICS.txt"))
    {
        sorr_ios_write_text_file(diagnostics_readme_path,
                                 ...
                                 "Touch controls use the fixed layout; legacy ios_touch_controls.ini files are ignored.\n");
    }
    ...
    if (!sorr_ios_prepare_bundled_data(layout, app_support_dir)) return 0;
...
static void sorr_ios_run_d2_probe(sorr_ios_data_layout *layout)
{
    ...
    if (layout->bundled_data)
    {
        sorr_ios_status_clear();
        sorr_ios_status_add("BUNDLED GAME DATA");
        staging_status = "bundled (no import required)";
    ...
    if (layout->bundled_data)
    {
        sorr_ios_status_add("GAME ASSETS READY");
    }
    else if (strcmp(staging_status, "copied") == 0)
    {
