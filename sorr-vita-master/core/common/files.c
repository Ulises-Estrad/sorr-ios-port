...
file * file_open( const char * filename, char * mode )
{
    ...
    PORTABLE_DIAG_LOG( "FILE", "open request filename=%s mode=%s", filename ? filename : "(null)", mode ? mode : "(null)" );
    ...
    {
        opened_files++;
        PORTABLE_DIAG_LOG( "FILE", "open ok direct name=%s mode=%s", filename, mode );
        return f ;
    }
         ...
         !strchr( mode, '+' ) && !strchr( mode, 'w' ) )
    {
        for ( i = 0 ; i < x_files_count ; i++ )
        {
            if ( strcmp( filename, x_file[i].name ) == 0 )
            {
                ...
                opened_files++;
                PORTABLE_DIAG_LOG( "FILE", "open ok embedded name=%s stub=%s", filename, x_file[i].stubname );
                return f ;
            }
        }
    }
...
/*    while (( c = *filename ) )
    ...
    *p = '\0';
    ...
    if ( strchr( name, '.' ) )
    {
        ...
        {
            opened_files++;
            PORTABLE_DIAG_LOG( "FILE", "open ok path=%s requested=%s mode=%s", here, filename, mode );
            return f ;
        }
    }*/
    ...
    PORTABLE_DIAG_LOG( "FILE", "open fail filename=%s mode=%s", filename, mode );
    free( f ) ;
    return 0 ;
}
...
void file_addp( const char * path )
{
    ...
    PORTABLE_DIAG_LOG( "FILE", "add path %s", truepath );
}
