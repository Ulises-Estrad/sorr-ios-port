...
int main( int argc, char *argv[] )
{
  ...
  SDL_setenv("VITA_DISABLE_TOUCH_BACK", "1");
    ...
    PORTABLE_DIAG_LOG( "LOOP", "main enter argc=%d argv0=%s", argc, argc > 0 ? argv[0] : "(null)" );
    ...
    if ( !embedded )
    {
        ...
        PORTABLE_DIAG_LOG( "LOOP", "dcb_load begin %s", dcbname );
        if ( !dcb_load( dcbname ) )
        {
            ...
            while ( dcbext && *dcbext )
            {
                ...
                PORTABLE_DIAG_LOG( "LOOP", "dcb_load fallback begin %s", dcbname );
        ...
        PORTABLE_DIAG_LOG( "LOOP", "dcb_load ok %s", dcbname );
    }
    else
    {
        PORTABLE_DIAG_LOG( "LOOP", "dcb_load_from embedded begin %s offset=%d", dcbname, dcb_signature.dcb_offset );
        dcb_load_from( fp, dcbname, dcb_signature.dcb_offset );
        PORTABLE_DIAG_LOG( "LOOP", "dcb_load_from embedded ok %s", dcbname );
    }
    ...
    PORTABLE_DIAG_LOG( "LOOP", "sysproc_init begin" );
    sysproc_init() ;
    PORTABLE_DIAG_LOG( "LOOP", "sysproc_init end" );
    ...
    argv[0] = filename;
    PORTABLE_DIAG_LOG( "LOOP", "bgdrtm_entry begin filename=%s argc=%d", filename, argc );
    bgdrtm_entry( argc, argv );
    PORTABLE_DIAG_LOG( "LOOP", "bgdrtm_entry end mainproc=%p", mainproc );

    if ( mainproc )
    {
        PORTABLE_DIAG_LOG( "SCRIPT", "instance_new mainproc=%s", mainproc->name );
        mainproc_running = instance_new( mainproc, NULL ) ;
        PORTABLE_DIAG_LOG( "LOOP", "instance_go_all begin main instance=%p", mainproc_running );
        ret = instance_go_all() ;
        PORTABLE_DIAG_LOG( "LOOP", "instance_go_all end ret=%d", ret );
    }
